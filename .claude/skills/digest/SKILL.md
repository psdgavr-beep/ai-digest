...
## Шаг 0. Определить тему

Если `$ARGUMENTS` непустой — используй его как тему и переходи к шагу 1.

Если `$ARGUMENTS` пустой:
1. Прочитай файл `.claude/news-ideas.md`
2. Найди первую тему из раздела `## <дата>` которая ещё НЕ находится в разделе `## Использованные`
3. Запомни текст этой темы (заголовок + описание) — это будет тема для статьи
4. Перенеси эту тему в конец файла в раздел `## Использованные` (создай его если нет), удалив из исходного раздела
5. Если свежих тем нет совсем — запусти субагент **news-scout** чтобы он пополнил список, затем повтори с пункта 1
